...
class CharacterInfoAgent(BaseAgent):
    ...
    # Canonical name mappings for known characters
    CHARACTER_MAPPINGS: Dict[str, Dict[str, str]] = {
        ...
        "natasha romanoff": {
            "wikipedia": "Black_Widow_(Natasha_Romanova)",
            "marvel_fandom": "Natalia_Romanova_(Earth-616)",
        },
        "black widow": {
            "wikipedia": "Black_Widow_(Natasha_Romanova)",
            "marvel_fandom": "Natalia_Romanova_(Earth-616)",
        ...
        "thor odinson": {
            "wikipedia": "Thor_(Marvel_Comics)",
            "marvel_fandom": "Thor_Odinson_(Earth-616)",
        },
        "thor": {
            "wikipedia": "Thor_(Marvel_Comics)",
            "marvel_fandom": "Thor_Odinson_(Earth-616)",
        ...
        "reed richards": {
            "wikipedia": "Mister_Fantastic",
            "marvel_fandom": "Reed_Richards_(Earth-616)",
        },
        "victor von doom": {
    ...
    def __init__(self):
        ...
        self.sources = {
            ...
            "marvel_fandom": "https://marvel.fandom.com/wiki/",
        }
    ...
    async def execute(
        ...
        mapping = self.CHARACTER_MAPPINGS.get(normalized_name)

        for source_key, source_base in self.sources.items():
            if mapping and source_key in mapping:
                # Use canonical mapping
                wiki_page = mapping[source_key]
                url = f"{source_base}{wiki_page}"
                logger.info(f"Using canonical mapping: {character_name} -> {url}")
            else:
    ...
    def _parse_infobox(
        ...
        for row in rows:
            ...
            if "full name" in header_text or "real name" in header_text:
                ...
                alias_list = [
                    a.strip() for a in re.split(r"[,;]|\band\b", data_text) if a.strip()
                ]
                ...
                occupation_list = [
                    o.strip() for o in re.split(r"[,;]|\band\b", data_text) if o.strip()
                ]
    ...
    def _extract_universe(self, source_base: str, soup: Tag) -> str:
